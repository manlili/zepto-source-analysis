...
(function (global, factory) {
  ...
  var Zepto = (function () {
    var undefined, key, $, classList, emptyArray = [], concat = emptyArray.concat, filter = emptyArray.filter, slice = emptyArray.slice,
      ...
      // 取出html中的第一个标签 比如<div>test</div>里面的div
      fragmentRE = /^\s*<(\w+|!)[^>]*>/,
      // 匹配标签，不匹配标签里面的内容，如匹配<img><p></p>,不匹配<p>123</p>
      singleTagRE = /^<(\w+)\s*\/?>(?:<\/\1>|)$/,
      // 全局区分大小写匹配<但是它后面却不是单标签  比如img br input
      tagExpanderRE = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/ig,
      // 忽略大小写查找以body或者html开头的数据，但是不捕获body或者html
  ...
  // it compatible with browsers that don't support the DOM fully.
    zepto.fragment = function (html, name, properties) {
      /**
      * @html: html字符串
      * @name: 通过name可在containers中查找容器节点，如果不传入，取得的容器默认是div
      * @properties： 节点属性对象
      */
      var dom, nodes, container

      //  singleTagRE = /^<(\w+)\s*\/?>(?:<\/\1>|)$/     匹配标签，不匹配标签里面的内容，如匹配<img><p></p>,不匹配<p>123</p>
      // 如果是单标签，直接用单标签创建元素
      if (singleTagRE.test(html)) dom = $(document.createElement(RegExp.$1))

      // 如果html不是单标签
      if (!dom) {
        // tagExpanderRE = /<(?!area|br|col|embed|hr|img|input|link|meta|param)(([\w:]+)[^>]*)\/>/ig  全局区分大小写匹配<但是它后面却不是单标签  比如img br input
